...
func AddKubeletConfigFlags(fs *pflag.FlagSet, c *kubeletconfig.KubeletConfiguration) {
	...
	fs.StringVar(&c.CPUManagerPolicy, "cpu-manager-policy", c.CPUManagerPolicy, "<Warning: Alpha feature> CPU Manager policy to use. Possible values: 'none', 'static', 'pool'. Default: 'none'")
	...
	fs.Var(flag.NewMapStringString(&c.CPUPools), "cpu-pools", "A set of CPU to pool assignments for the 'pool' CPU manager policy which define how CPU cores should be split to named CPU pools (e.g. reserved=1-2,default=*). There is no default setting for this option.")
}

...
func AddKubeletConfigFlags(fs *pflag.FlagSet, c *kubeletconfig.KubeletConfiguration) {
	...
	fs.StringVar(&c.CPUManagerPolicy, "cpu-manager-policy", c.CPUManagerPolicy, "<Warning: Alpha feature> CPU Manager policy to use. Possible values: 'none', 'static', 'pooled'. Default: 'none'")
	...
	fs.Var(flag.NewMapStringInts(&c.CPUPools), "cpu-pools", "A set of CPU to pool assignments, for the 'pooled' CPU manager policy, in the form of poolname0=pool0cpu0-pool0cpu1,poolname1=... (comma-separated list supported as well). This is used to split up the available CPUs/cores to named pools. There is no default setting for this option.")
}
